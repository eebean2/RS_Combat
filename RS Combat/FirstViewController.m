...
@interface FirstViewController () {
    ...
    NSInteger intRange;
    NSInteger intPrayer;
    NSInteger intSummoning;
    NSInteger intConstitution;
...
- (void)viewDidLoad {
    ...
    summoningText.inputAccessoryView = keyboardDoneButtonView;
    constitutionText.inputAccessoryView = keyboardDoneButtonView;
...
- (IBAction)work:(id)sender {
    ...
    intSummoning = [summoningText.text integerValue];
    intConstitution = [constitutionText.text integerValue];
    [self performSegueWithIdentifier:@"fPush" sender:self];
}

- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if ([segue.identifier isEqualToString:@"fPush"]) {
        F2PViewController *controller = (F2PViewController *)segue.destinationViewController;
        ...
        controller.summoningNumber = &(intSummoning);
        controller.constitutionNumber = &(intConstitution);
    }
}
